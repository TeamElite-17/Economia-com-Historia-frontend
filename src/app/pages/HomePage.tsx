...
export function HomePage() {
  ...
  // Conteúdos carregados do backend (fallback: dados mock se offline)
  const [allContent, setAllContent] = useState<ContentItem[]>([...CONTENT_ITEMS]);

  useEffect(() => {
    // Busca conteúdos publicados directamente do backend para garantir que
    // vídeos/podcasts recém publicados aparecem sem precisar de reload
    const API_BASE = import.meta.env.VITE_API_BASE_URL ?? '/api';
    const token = localStorage.getItem('economia-historia-token');
    fetch(`${API_BASE}/v1/content-items`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
      .then(data => {
        const items = extractArray<Record<string, unknown>>(data);
        if (items.length > 0) {
          const mapped = items.map(item => mapContentItem(item));
          // Actualiza o array global também (para outras partes da app)
          CONTENT_ITEMS.splice(0, CONTENT_ITEMS.length, ...mapped);
          setAllContent(mapped);
        }
      })
      .catch(() => {
        // Fallback silencioso: mantém o que estava em CONTENT_ITEMS
        setAllContent([...CONTENT_ITEMS]);
      });
  }, []);

  const featuredContent = allContent.find(c => c.featured && c.isJindungo) || allContent[0] || null;
  const jindungoContent = allContent.filter(c => c.isJindungo);
  const filteredContent = activeCategory === 'Todos'
    ? allContent.filter(c => c.status === 'published')
    : allContent.filter(c => c.category === activeCategory && c.status === 'published');
    ...
    <div className="pb-12">
      ...
      <section className="relative h-[380px] sm:h-[420px] md:h-[500px] overflow-hidden">
        {featuredContent ? (
          ...
          <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #5C1520 0%, #7B1D2D 55%, #C9A84C 100%)' }} />
        )}
        <div
            ...
            {featuredContent?.isJindungo && (
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white" style={{ backgroundColor: '#D64E12' }}>
                  <Flame size={12} /> <span className="hidden sm:inline">Jindungo â€” Conteúdo em Destaque</span><span className="sm:hidden">Jindungo</span>
                </span>
              </div>
            )}
            <h1 className="text-white text-xl sm:text-2xl md:text-4xl leading-tight mb-2 sm:mb-3">
              {featuredContent?.title ?? 'A plataforma está pronta para explorar'}
            </h1>
            <p className="text-white/80 text-sm md:text-base mb-4 sm:mb-6 line-clamp-2">
              {featuredContent?.description ?? 'Os conteúdos já estão disponíveis e podem ser navegados sem esperar pelo carregamento.'}
            </p>
      ...
      <div className="px-4 sm:px-6 md:px-8 pt-6 sm:pt-8 max-w-7xl mx-auto">
        ...
        {/* Jindungo Section — só aparece se houver conteúdo jindungo real */}
        {jindungoContent.length > 0 && (
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 p-8">
              <div className="flex-1 text-center md:text-left">
                ...
                {QUIZZES.length > 0 ? (
                  ...
                  <p className="text-white/70 text-sm mb-4">Nenhum quiz disponível de momento.</p>
                )}
              </div>
